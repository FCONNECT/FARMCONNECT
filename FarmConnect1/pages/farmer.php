

<!DOCTYPE html>
<html>
<head>
	<title>Farmers panel</title>
	<link rel="stylesheet" type="text/css" href="../style.css">
</head>
<body>
	<div class="logotop logo">
		<img src="../images/farmconnectlogo.png" >
	</div>

	<div class="navigation" >
			<ul>
				
				<li><a href="../activeUsers/homeactive.php">HOME</a></li>
				<li><a href="makert.php">MAKERT</a></li>				
				<li> <a href="myfarm.php">My Farm</a></li>
				<li><a href="about.html">ABOUT FC</a></li>
				<li><a href="contacts.php">CONTACTS</a></li>
				<li><a href="./pages/logout.php">LOGOUT</a></li>
				

				<!-- <li class="navbutton"><form method="POST" action="./pages/login.php"><input type="submit" name="logout" value="Logout" ></form></li> -->
				
			</ul>
	</div>


	<div class="footer">
       	
        	<hr style="border: 1px solid black">
           <p style="text-align: center"> Copyright &copy; <script>document.write(new Date().getFullYear())</script> FARMCONNECT All Rights Reserved</p>
       </div>

</body>
</html>